...
The [Transformer model](https://arxiv.org/abs/1706.03762)'s attention mechanism is now the underlying technology of most large and best performing ML models these days. So I came across [Andrej Karpathy](https://karpathy.ai/)'s implementation of [nanoGPT](https://github.com/karpathy/nanoGPT) and decided to dissect it and detail its step-by-step implementation. 

In 2017, the original [Transformer]({{ site.baseurl }}{% post_url 2020-05-28-AI-Supercomputing-2 %}) model in the paper [Attention is all you need](https://arxiv.org/abs/1706.03762), presented a tool for translation based on a attention mechanism that was not recursive (with RNNs) but with an attention matrix that exposes all tokens to all other tokens in a sequence. Similarly to previous translation methods it had two main components: an encoder that learns the structure of the source language, and a decode that learns the translation mechanism to the new language. We soon realized that the Transformer had much more to offer and could be used in many datatypes (time-sequences, images, etc). And most importantly, that the components could be "stacked" as a sequence of ML blocks, to perform higher-level cognitive tasks.
...
Note, for simplicity, we'll use the following notation in shape of arrays:
- shape `C` for the *embedding size* of each token, defined by the `n_embed` on top;
- shape `B` for the *batch size* defined by `batch_size` above;
- shape `T` for the *time* dimension, or input sequence length, defined by `block_size`.  
...
For a faster memory-efficient implementation of the attention mechanism, see [Flash Attention](https://arxiv.org/abs/2205.14135).
