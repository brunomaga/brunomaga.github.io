...
and this will start blabbing some text, which looks very random for now:
...
This is because for the sake of this tutorial, we used very small parameters for our tiny GPT model. Better results can be achieved by increasing several of the parameters above, such as batch size, larger attention and more GPT blocks. As an example, if you try the following:
...
If you are interest in the real model size, the paper [Language Models are Few-Shot Learners](https://arxiv.org/abs/2005.14165) provide us with several details of the real implementation of GPT-3:
...
The task that we detailed in this post is *only* the training step, the pre-training. This is only the initial step that lets the model learn text structure from a general dataset e.g. wikipedia or our tiny shakespeare. For specific task resolutions, two other stages of finetuning should follow, that optimize the model to the task at hand (e.g. question-answer tasks). In the original paper, this involved using reinforcement learning against user feedback.
