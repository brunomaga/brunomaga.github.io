...
class GPTlite(nn.Module):
  ...
  def __init__(self, num_head, n_layer):
    super().__init__()
...
```python
m  = GPTlite(num_head=n_head, n_layer=n_layer).to(device)
```
...
All done! For the full python implementation of the code presented, see the file <a href="/assets/GPT-lite/gptlite.py">gptlite.py</a> of the following [GPT-lite repo](https://github.com/brunomaga/gptlite/tree/main/GPT-lite).
