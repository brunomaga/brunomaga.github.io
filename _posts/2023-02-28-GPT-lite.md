...
An example of such model is [BERT]({{ site.baseurl }}{% post_url 2020-05-28-AI-Supercomputing-2 %}), a stack of Transformer encoders, used for regression and classification tasks. Here we will look at a decoder-only architecture (*a la [GPT-2](https://en.wikipedia.org/wiki/GPT-2)*), used for generating long sentences from a given input. The fundamental difference between the encoder and the decoder is the *mask* in their attention mechanism -- we'll cover it later.
