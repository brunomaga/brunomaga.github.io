---
layout: post
title:  "Building a small GPT in Python and C++"
categories: [machine learning, gpt, C]
...
Back to data preparation, we will split create the tokens for our input dataset and do a 90%-10% train/validation data split:
...
class MyModel(nn.Module):
  def __init__(self, num_head=4, n_layer=3):
    ...
    # nn.Embedding is for categorical input. nn.Linear is for ordinal input.
    self.token_embedding_table = nn.Embedding(vocab_size, n_embd)
    self.position_embedding_table = nn.Embedding(block_size, n_embd)
    #self.sa_head = Head(n_embd) #single attention head!
    
    #self.sa_heads = MultiHeadAttention(num_heads, n_embd//num_heads) #multiple attention heads
    #self.ffwd = FeedForward(n_embd) 
    ...
    self.ln = nn.LayerNorm(n_embd) #one final layer norm at the end of the transformer and before linear layer that oututs the vocabulary
...
and we can now instantiate is and copy it to the GPU:
...
We initialize the train loop, no magic here:
...
for steps in range(max_iters):
  ...
  if steps % eval_interval == 0:
    ...
    @torch.no_grad()
    def eval_loss():
      # tell pytorch we dont intend to do backprop on this data, saves storing all intermediatte variables
...
Now test the model. Pass a single token (the `\n` character, encoded as `0`) to the model, and generate a sequence of 1000 tokens: 
...
print(decode(m.generate(idx, max_new_tokens=1000).tolist()[0]))
```
